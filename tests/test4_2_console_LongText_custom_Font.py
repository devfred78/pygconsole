...
"""
	...
	Test 4_2 : Write a long text on the console with an external font, and test the clear function.
	ESCAPE Key to exit.
"""
...
EXTERNAL_FONT = os.path.join(RESOURCE_DIR,"clacon2.ttf") # External font

DISPLAY_SIZE = Coordinates(1920,1080) # screen resolution
DEFAULT_CONSOLE_COORDINATES = Coordinates(50,550) # upper left corner coordinates of the default console
CONSOLE_COORDINATES = Coordinates(1000,550) # upper left corner coordinates of the external font console
...
def main():
	...
	default_console = pygconsole.console.Console.get_console(name = "default", font_size = 17, logger = log)
	console = pygconsole.console.Console.get_console(font_size = 17, logger = log)
	console.change_font({"normal":EXTERNAL_FONT})
	...
	while True:
		...
		for event in pygame.event.get():
			...
			if event.type == KEYDOWN:
				if event.key == K_ESCAPE: sys.exit()
					...
					default_console.add_char(LONG_TEXT)
		...
		# Console display
		screen_surface.blit(default_console.surface,DEFAULT_CONSOLE_COORDINATES)
